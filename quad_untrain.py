...
# Define quadratic function: y = 1.5x1^2 - 2x2^2 + 0.8x1*x2 + 0.5x1 + x2 + 3
def quadratic_func(x):
    x1, x2 = x[:, 0], x[:, 1]
    y = 1.5 * x1**2 - 2.0 * x2**2 + 0.8 * x1 * x2 + 0.5 * x1 + x2 + 3.0
    return y.unsqueeze(1)

# Generate data: 2D inputs in [-2, 2]^2
n_samples = 50
X_raw = 4 * torch.rand(n_samples, 2, dtype=dtype) - 2
Y_raw = quadratic_func(X_raw)
...
# Define 2–3–3–1 model with Softplus and no bias
class SimpleNet2D(nn.Module):
    def __init__(self):
        super().__init__()
        self.W1 = nn.Parameter(torch.randn(3, 2, dtype=dtype))
...
# Save weights after 44 examples
...
x46 = torch.zeros((1, 2), requires_grad=True, dtype=dtype)
y46 = torch.zeros((1,), requires_grad=True, dtype=dtype)

unroll_net = SimpleNet2D()
with torch.no_grad():
...
for step in range(600):
    ...
    if step % 100 == 0 or step == 599:
        print(f"Step {step} | Meta Loss: {meta_loss.item():.8f} | x46: {x46.data.numpy()}, y46: {y46.item():.6f}")

print("\n🎯 Constructed anti-example:")
print(f"x46 = {x46.data.numpy()}, y46 = {y46.item():.6f}")

# Step 4: Train on anti-example
opt.zero_grad()
y_pred_46 = net(x46)
